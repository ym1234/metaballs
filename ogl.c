...
#define die(...) do { printf("Something wrong happen existing, line: %d\n", __LINE__); exit(-1); } while (0);
...
Window create_window(Display *d) {
    ...
    XSelectInput(d, win, KeyPressMask | ExposureMask | StructureNotifyMask | PropertyChangeMask	| ButtonPressMask);
    if(!win) {
        die("window");
    }
    XFree(vi);
	...
	atom = XInternAtom(d, "WM_DELETE_WINDOW", False);
	XSetWMProtocols(d, win, &atom, 1);
